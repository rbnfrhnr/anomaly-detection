...
def Load_Dataset(args):
    ...
    target_df = target_df[:int(0.7*len(target_df))]
    test_df = target_df[int(0.7*len(target_df)):]
    
    target_norm_df = target_df[target_df["label_num"]==0]
    kisti_abnorm_rate = Counter(target_df["class"])["abnorm"] / len(target_df)

    same_column = list(target_df.columns)
    if args.normal_select:
        traindataset = NetworkDataset_ae(target_df, "train",  args.model, same_column, args.preprocess, args.total_seq, \
                                            args.overlap, 5, 'transfer')
    else:
    ...
    validdataset = NetworkDataset_ae(target_df, "valid", args.model, same_column, args.preprocess, args.total_seq,\
                                        args.overlap, 5, "transfer")
    ...
    if args.only_infer:
        ...
        for e in range(args.epochs):
            ...
            while i < len_dataloader:
                ...
                target_bce_lst.append(target_bce.mean(1))
