...
# def make_conn_dataframe(conn_lst):
#     dfs = []
#     for i, c in enumerate(conn_lst):
#         for file in os.listdir('../ctu-13/CTU-13-Dataset/'+c):
#             if file.endswith(".binetflow"):
#                 if i==0:
#                     dfs.append(pd.read_csv('../ctu-13/CTU-13-Dataset/'+ c + '/' + file, delimiter=","))
#                 else:
#                     dfs.append(pd.read_csv('../ctu-13/CTU-13-Dataset/'+ c + '/' + file, delimiter=",", skiprows=0))
#     df = pd.concat(dfs)
#     return df
...
possible_augments = ['flip', 'crop-resize', 'gauss-noise']


def augment(df, augments):
...
def preprocess_stat_KISTI(df, period_len, rm_ntp):
    '''medium dataset
       stat(1 min) 
       use port, ip addr'''
    # df = df.loc[~df["service"].isin(['mysql', 'imap','ftp','ftp-data'])]
    ...
    df.duration = df.apply(lambda x: 0 if x.duration == '-' else x.duration, axis=1)
    df.duration = df.duration.astype(float)
    df_filtered = df.filter(['duration', 'orig_pkts', 'orig_ip_bytes', \
                             'proto', 'service', "time_chunk", 'id.orig_h', 'id.resp_p', 'id.orig_p', 'id.resp_h'],
                            axis=1)
    ...
    grouped = df_filtered.groupby(["time_chunk", "id.orig_h"])
    df_ipcnt = pd.DataFrame(grouped["id.orig_h"].agg('count')).rename(columns={'id.orig_h': "sripcnt"}).reset_index()
    result = grouped['duration', 'orig_pkts', 'orig_ip_bytes'].aggregate([np.mean, np.std])
    result_service = df_filtered.groupby(["time_chunk", "id.orig_h", "service"]).size().unstack(
        fill_value=0).reset_index(level=[0, 1])
    result_service = result_service.drop(["-"], axis=1)
    result_proto = df_filtered.groupby(["time_chunk", "id.orig_h", "proto"]).size().unstack(fill_value=0).reset_index(
    ...
    df_fill_oh.loc[:, normalize_column] = \
        df_fill_oh.loc[:, normalize_column].div(df_fill_oh.loc[:, normalize_column].sum(axis=1), axis=0)
    normalize_column = ['icmp', 'tcp', 'udp', 'arp', 'igmp', 'rtp']  # protocol
    for c in normalize_column:
        if c not in df_fill_oh.columns:
            df_fill_oh[c] = 0
    df_fill_oh.loc[:, normalize_column] = \
...
def port_service_rate(df):
    service_uni = df.service.unique()
    for i in service_uni:
        if len(i) > 0:
            cnt = Counter(df[df.service == i].Dport.values)
            service_sum = sum(list(cnt.values()))
            for k, v in cnt.items():
                cnt[k] = v / service_sum
            df_filtered = df[df["service"] == i]
            df_filtered.loc[:, "service_p_rate"] = df_filtered.apply(lambda x: cnt[x.Dport], axis=1)
            df.loc[df_filtered.index, "service_p_rate"] = df_filtered["service_p_rate"]
    return df
